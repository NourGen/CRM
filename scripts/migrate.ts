...
async function runMigration() {
  ...
  for (const row of bankRows) {
    try {
      ...
      // Sheet header is "Correct (1-4)" (1-based); questionsJson convention is 0-based
      const rawCorrect = parseNumber(row['Correct (1-4)'] ?? row.Correct);
      q.correct = rawCorrect && rawCorrect >= 1 ? rawCorrect - 1 : rawCorrect;
