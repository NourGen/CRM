"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SupportRequest = void 0;
const typeorm_1 = require("typeorm");
const user_entity_1 = require("./user.entity");
let SupportRequest = class SupportRequest {
    id;
    legacyId;
    agentLegacyId;
    agent;
    agentName;
    clientName;
    clientPhone;
    clientOc;
    comment;
    status;
    managerResult;
    createdAt;
    resolvedAt;
};
exports.SupportRequest = SupportRequest;
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)('uuid'),
    __metadata("design:type", String)
], SupportRequest.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'legacy_id', type: 'varchar', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "legacyId", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'agent_legacy_id', type: 'varchar', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "agentLegacyId", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => user_entity_1.User, { nullable: true, onDelete: 'SET NULL' }),
    (0, typeorm_1.JoinColumn)({ name: 'agent_id' }),
    __metadata("design:type", user_entity_1.User)
], SupportRequest.prototype, "agent", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'agent_name', type: 'varchar', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "agentName", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'client_name', type: 'varchar', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "clientName", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'client_phone', type: 'varchar', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "clientPhone", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'client_oc', type: 'varchar', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "clientOc", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'text', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "comment", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'varchar', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "status", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'manager_result', type: 'text', nullable: true }),
    __metadata("design:type", String)
], SupportRequest.prototype, "managerResult", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'created_at', type: 'timestamp', nullable: true }),
    __metadata("design:type", Date)
], SupportRequest.prototype, "createdAt", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'resolved_at', type: 'timestamp', nullable: true }),
    __metadata("design:type", Date)
], SupportRequest.prototype, "resolvedAt", void 0);
exports.SupportRequest = SupportRequest = __decorate([
    (0, typeorm_1.Entity)('support_requests')
], SupportRequest);
//# sourceMappingURL=support-request.entity.js.map